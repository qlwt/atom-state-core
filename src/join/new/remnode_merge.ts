...
import type { Join_RemNodeMerge } from "#src/join/type/remnode_merge.js"
...
type Joins_Props<RDef extends RemNode_Def, Joins extends Join_NewRemNodeMerge_Joins_G<RDef>> = {
    [K in keyof Joins]: sc.OSignal<Join_Option<
        sc.Signal_InferO<ReturnType<ReturnType<Joins[K]>["prop"]>>
    >>
}

export type Join_NewRemNodeMerge_Joins_G<RDef extends RemNode_Def> = {
    readonly [K in string]: () => Join<RemView<RDef>, any>
}

export type Join_NewRemNodeMerge_Joins<RDef extends RemNode_Def, Joins extends Join_NewRemNodeMerge_Joins_G<RDef>> = {
    [K in keyof Joins]: Join_Option_InferValue<sc.Signal_InferO<ReturnType<ReturnType<Joins[K]>["prop"]>>>
}

export type Join_NewRemNodeMerge_Params<
    Param,
...
export const join_new_remnode_merge = function <
    Param,
    RDef extends RemNode_Def,
    Joins extends Join_NewRemNodeMerge_Joins_G<RDef>
>(
    params: Join_NewRemNodeMerge_Params<Param, RDef, Joins>
): Join<Param, Join_RemNodeMerge<RDef, Join_NewRemNodeMerge_Joins<RDef, Joins>>> {
    return {
        root: param => {
            ...
            const joins_s = sc.osignal_new_pipe(joins_rawprops_s, (joins_rawprops): Join_Option<Join_NewRemNodeMerge_Joins<RDef, Joins>> => {
                const joins: Partial<Join_NewRemNodeMerge_Joins<RDef, Joins>> = {}
                ...
                return {
                    kind: Join_Option_Kind.View,
                    value: joins as Join_NewRemNodeMerge_Joins<RDef, Joins>,
                }
            })
...
export const join_newf_remnode_merge = function <
    Param,
    RDef extends RemNode_Def,
    Joins extends Join_NewRemNodeMerge_Joins_G<RDef>
>(
    params: Join_NewRemNodeMerge_Params<Param, RDef, Joins>
): JoinF<Param, Join_RemNodeMerge<RDef, Join_NewRemNodeMerge_Joins<RDef, Joins>>> {
    const result = join_new_remnode_merge(params)

    return () => result
}

export const join_news_remnode_merge = function <
    Param,
    RDef extends RemNode_Def,
    Joins extends Join_NewRemNodeMerge_Joins_G<RDef>
>(
    ...
    const result = join_new_remnode_merge({
        link_new,
        joins,
    })
